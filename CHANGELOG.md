...
- App icon replaced with the project logo (`build/icon.png`, sourced from
  `deepseek harness.png`); electron-builder generates the Windows `.ico` from
  it at build time, so the window, taskbar and installer share the new look.
...
- Package metadata author email set back to the project contact email.
