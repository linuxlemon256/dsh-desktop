...
- **端口设置界面** —— 在 dsh 设置页「Web UI 插件」栏目里直接改服务端口、扫描端口占用；启动时若端口被其他程序占用，会提供空闲端口
...
- [x] ~~窗口打开期间 `dsh web` 崩溃时自动重启~~ —— 已在 2.0.3 完成
- [x] ~~设置界面（端口）~~ —— 已在 2.0.3 完成：dsh 设置页「Web UI 插件」栏目里有端口卡片（改端口、扫描占用）
- [ ] 设置界面（开机自启）
- [x] ~~安装提速~~ —— 已在 2.0.x 完成：裁剪内置运行时（去掉 `.d.ts`/`.map`/源码/文档/测试），NSIS 改用 `useZip`（直接解压、无临时暂存），安装耗时缩短约 3.5 倍
